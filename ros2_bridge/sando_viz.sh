#!/usr/bin/env bash
# Headless demo: bring up the D435i closed loop, fly one goal, render a PNG.
# Output: /tmp/sando_flight.png (+ copy to $HOME/sando_flight.png).
# Paths derive from this script's location; workspace defaults to ~/code/sando_ws
# (override: SANDO_WS=/path/to/ws bash sando_viz.sh).
...
python3 -c "import matplotlib" 2>/dev/null || {
  echo "matplotlib missing — install it first:  sudo apt-get install -y python3-matplotlib"; exit 1; }
...
python3 "$BRIDGE_DIR/record_and_plot.py" 15.0

cp -f /tmp/sando_flight.png "$HOME/sando_flight.png" 2>/dev/null && echo "copied to $HOME/sando_flight.png"
